...
public class SimpleCustomTasksImpl implements SimpleCustomTasks {
	...
	private void executWindowsProcedure(FullConfiguration configuration) {
		...
		if (attempts >= maxAttempts || !processInSuccess) {
			System.err.println("Windows process failed to complete suceefully after: " + attempts
					+ " attempts, silently skipping.");
		}
			...
			while ((line = br.readLine()) != null) {
				...
				while (!lineProcessed) {
					...
					try {
						...
						if (currentProc.exitValue() == 0 || currentCmdExecutionAttempt > cmdExecutionAttempt) { // Exit
																												// code
																												// OK,
																												// OR
																												// max
							...
							if (currentProc.exitValue() == 128 && line != null
									&& line.toLowerCase().contains("taskkill")) { // When process not running taskkill
																					// returns code 128
								lineProcessed = true;
							} else {
								System.err.println(line + " EXITED WITH ERROR CODE: " + currentProc.exitValue()); // To
																													// better
																													// handle
																													// future
																													// errors
						...
						System.err.println(line + "ERROR WITH BELOW STACK");
						e.printStackTrace();
